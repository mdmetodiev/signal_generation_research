...
def generate_signals(
...
) -> Tuple[np.ndarray, np.floating[Any], np.floating[Any]]:
    ...
    # Apply quantile cuts
    up_q = np.quantile(y_pred, qmax)
    low_q = np.quantile(y_pred, qmin)
...
def walk_forward(
    ...
    while current_test_end < end_date + test_period:
        ...
        X_emb_train = pca.fit_transform(X_train_clean)
        ...
        X_emb_test = pca.transform(X_test_clean)
    ...
    return final_model, mod_preds, df_trading
